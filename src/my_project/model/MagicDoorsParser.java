...
public class MagicDoorsParser implements Parser {

    private MagicDoorsScanner scanner;

    public MagicDoorsParser(){
        scanner = new MagicDoorsScanner();
    }
    ...
    public boolean parse(String input) {
        if(scanner.scan(input)) {
            while (scanner.getType().equals("A-START")) {
                scanner.nextToken();
                if (scanner.getType().equals("A-END")) {
                    scanner.nextToken();
                    if (scanner.getType().equals("NODATA")) return true;
                }
            }
            while(scanner.getType().equals("B-START")) {
                scanner.nextToken();
                if (scanner.getType().equals("B-END")) {
                    scanner.nextToken();
                    if (scanner.getType().equals("NODATA")) return true;
                }
            }
